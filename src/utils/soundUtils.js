  ...
  try {
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch(error => {
          console.error(`Error playing ${soundType} notification sound:`, error);
          // Attempt to play the sound again with user interaction
          console.log(`Sound could not be played automatically due to browser policy`);
...
const forcePlaySoundWithUserInteraction = (soundFile) => {
  ...
  soundButton.addEventListener('click', () => {
    const audio = new Audio(soundFile);
    audio.volume = 0.5;
    audio.play().catch(err => console.error("Still couldn't play sound:", err));
...
export const playDeleteSound = () => {
  const audio = new Audio('/audio/cylindrical-wrench-bell-2-170504_1501-46956.mp3');
  audio.volume = 0.5; // Set volume to 50%
  audio.play().catch(error => {
